...
以前はCTA（コンバージョン型）の例文が「公式LINEに登録した方のみお受けしています。`https://lin.ee/Q0U8U3a`」だったが、**LINE登録という一段階多いアクションではなく、`https://career-uranai.site/shindan`（簡易診断ページ）へ直接誘導する形に統一した**。理由は、リンクを踏んだその場で反応が得られる方が導線として自然で、登録という追加ハードルを挟まない方が良いというユーザー判断。Workflow 02・04（`Claudeで投稿案生成`/`Claudeで再生成`）・06（`Claudeでテンプレート投稿生成`）の全てで同じリンクに統一済み。
