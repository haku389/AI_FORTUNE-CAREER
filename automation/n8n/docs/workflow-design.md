...
**ただしWorkflow 06（定期テンプレート投稿）はテンプレートごとに誘導先を出し分けている（2026-07-09）**。西洋占星術・九星気学のテンプレート（①星座別タイミング型・②共感型・③ランキング型(星座)・⑦本命星ランキング型）は、一般読者になじみが薄い専門用語（本命星名・星の配置等）を扱うため`https://career-uranai.site/guide`（西洋占星術・MBTI・九星気学の用語解説ページ、`app/guide/`配下にMBTI/西洋占星術/九星気学それぞれのサブページあり）に誘導する。一方MBTIのテンプレート（④全体ランキング型・⑤一部ランキング型・⑥職種×MBTI・⑧比較型）はMBTIタイプ名が一般にも比較的知られているため、従来通り`/shindan`（簡易診断）に誘導する。
...
### CTAリンクはテンプレートごとに出し分け（2026-07-09、同日中に修正）
Workflow 02・04は常に`/shindan`（簡易診断）に誘導する。Workflow 06は当初「全テンプレート`/guide`に統一」としたが、**MBTIは一般にも比較的知られている**というユーザー指摘を受けて修正。西洋占星術・九星気学系（①②③⑦、星の配置や本命星名など一般になじみが薄い用語を扱う）だけ`/guide`に誘導し、MBTI系（④⑤⑥⑧）は`/shindan`のまま。「分からない可能性が高い用語を扱っているかどうか」で判断し、テーマだけで一律に決めないこと（[§3](#3-workflow-02-sns投稿案生成)のCTAリンク方針も参照）。
