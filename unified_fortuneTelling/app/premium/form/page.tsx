...
export default function PrecisePage() {
  ...
  /* ── インアプリブラウザ検出（Google/Facebook等のWebView）── */
  useEffect(() => {
    const ua = navigator.userAgent
    // GSA = Google Search App, FBAN/FBAV = Facebook, Line = LINE in-app
    const inApp = /GSA\/|FBAN|FBAV|Instagram|Line\//.test(ua) || (ua.includes('wv') && ua.includes('Android'))
      ...
      <div style={{ ...pageStyle, justifyContent: 'center' }}>
        ...
        {!lineUserId ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0 }}>
            ...
            {/* Googleアプリ等インアプリブラウザの警告 */}
            {isInAppBrowser ? (
              <div style={{ background: '#1a1a0d', border: '1px solid #c8952a66', borderRadius: 12, padding: '16px', width: '100%', boxSizing: 'border-box', textAlign: 'center' }}>
                <div style={{ fontSize: 13, color: '#c8952a', fontWeight: 700, marginBottom: 8 }}>⚠️ ブラウザを切り替えてください</div>
                <div style={{ fontSize: 12, color: '#7888b8', lineHeight: 1.7, marginBottom: 12 }}>
                  このブラウザからはLINEログインができません。<br />
                  右上のメニューから<strong style={{ color: '#f0f4ff' }}>「Safariで開く」</strong>をタップしてください。
                </div>
                <div style={{ fontSize: 11, color: '#3a4870' }}>Safari / Chrome からアクセスしてください</div>
              </div>
            ) : (
              <a
                href="/api/line/login"
                style={{
                  ...
                  background: '#06c755', borderRadius: 12, padding: '16px 24px',
